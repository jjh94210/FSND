...
def create_app(test_config=None):
  ...
  def get_menus():
    ...
    return jsonify({
      ...
      'current_cuisine': list(set([menu['cuisine'] for menu in current_menus])),
      'cuisines': {
        cuisine.id: cuisine.type for cuisine in cuisines
      }
    })
  ...
  def create_menu(token):
    ...
    try:
      menu = Ourhome(menu=new_menu, description=new_description, cuisine=new_cuisine, preference=new_preference)
      menu.insert()
  ...
  def menus_search():
    ...
    try:
      selection = Ourhome.query.filter(Ourhome.menu.ilike('%{}%'.format(search)))
      current_menus = paginate_menus(request, selection)
