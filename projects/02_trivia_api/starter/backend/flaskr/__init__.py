...
def create_app(test_config=None):
  ...
  def after_request(response):
    response.headers.add('Access-Control-Allow-Headers', 'Content-Type, Authorizatoin, true')
  ...
  def quizzes():
    try:
      ...
      questions_pool = []
      if previous_questions is None or len(previous_questions) >= len(Question.query.filter_by(category=str(quiz_category['id'])).all()):
        questions_pool = Question.query.all()
      else:
        questions_pool = Question.query.filter(Question.id.notin_(previous_questions)).all()

      print(len(previous_questions))
      print(len(Question.query.filter_by(category=str(quiz_category['id'])).all()))

      questions = [question.format() for question in questions_pool]

      if quiz_category['id'] != 0:
        questions = []
        for question in questions_pool:
          if str(question.format()['category']) == quiz_category['id']:
